...
import { shortCaseId, useCase, useCases } from '../context/CaseContext'
        ...
        <>
          ...
          <div className="panel">
            <div className="section-head">
              ...
              <Link className="btn btn-secondary" to={`/investigations/${selectedId}?tab=coverage`}>Open case</Link>
            </div>
            {coverage.length === 0 ? (
                  ...
                  return (
                    <article className="nested-card" key={String(record.assessment_id ?? index)}>
                      <div className="section-head">
                        ...
                        <StatusBadge label={observed} tone={observed === 'detected' ? 'success' : observed === 'missed' ? 'failure' : 'info'} />
                      </div>
                      <p className="muted">Original suite: {String(record.original_suite ?? 'unknown')}</p>
                      <p>{String(record.notes ?? 'No notes recorded.')}</p>
